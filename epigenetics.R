...
###
tbl_data = read_xlsx(path = "Copy of Gene expression results GM19.xlsx",
                        sheet = 1)[,1:5]
tbl_data = rbind(tbl_data,read_xlsx(path = "Copy of Gene expression results GM19.xlsx",
                      sheet = 2)[,1:5])
tbl_data = rbind(tbl_data,read_xlsx(path = "Copy of Gene expression results GM19.xlsx",
                                    sheet = 3)[,1:5])
tbl_data = rbind(tbl_data,read_xlsx(path = "Copy of Gene expression results GM19.xlsx",
...
tbl_data = tbl_data[seq(4,192,4) ,]

names(tbl_data)[5] = "target"
names(tbl_data)[2] = "RapeOil"

targets = unique(tbl_data$`target gene`)
...
fml[[5]] = target~1   + SID  + RapeOil + I(RapeOil*(SID==0)) + I(RapeOil*(SID==1))  #+  f(DiettNo, model="iid")


adjacent = matrix(0, nrow = 4, ncol = 4)

adjacent[1,] = c(0,1,0,0)
adjacent[2,] = c(1,0,1,1)
adjacent[3,] = c(0,1,0,1)
adjacent[4,] = c(0,1,1,0)
g1 = inla.read.graph(adjacent)
plot(g1)

adjacent = matrix(0, nrow = 4, ncol = 4)

adjacent[1,] = c(0,1,0,0)
adjacent[2,] = c(1,0,0,1)
adjacent[3,] = c(0,0,0,1)
adjacent[4,] = c(0,1,1,0)
g2 = inla.read.graph(adjacent)
plot(g2)

adjacent[1,] = c(0,1,0,0)
adjacent[2,] = c(1,0,1,0)
adjacent[3,] = c(0,1,0,1)
adjacent[4,] = c(0,0,1,0)
g3 = inla.read.graph(adjacent)
plot(g3)
...
for (index in 1:length(targets)){
  ...
  idx = which(tbl_data$`target gene`==targets[index])
  ...
  plot1 = res %>% ggplot(mapping = aes(x = RapeOil, y = target, color = Size)) + ggtitle(paste0("               accid ",targets[index]," model ",j," pprob ",round(max(mliks),5))) +  geom_point() +
    geom_ribbon(aes(ymin = q_0.005quant, ymax = q_0.995quant, fill = Size, color = NULL), alpha = .25) + expand_limits(x = 0, y = 0)+
    geom_line(aes(y = q_mode), size = 1)+ theme(text = element_text(size = 20))  

  ggsave(plot = plot1,filename = paste0("plots_epig/epig_",targets[index],"_plot.png"))
  
  
  grid.arrange(plot1, plot2,plot3,plot4, ncol=2)
  
  
  reslist[[index]] = list(best = model.inla,probs = mliks, res = res, data.residuals = data.residuals, plot1 = plot1)
...
ress = as.data.frame(do.call(rbind, lapply(X = 1:length(reslist),function(i){
  ...
  saveRDS(file = paste0("models_epig/",targets[i],"_epi_best.RDS"),object = (reslist[[i]]$best$summary.fixed))
...
write.csv(ress,"models_epig/final_table.csv")
