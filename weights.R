...
###
tbl_data = read_xlsx(path = "salmon data.xlsx",
                        sheet = 1)[,c(3,4,6,7,9,11,13,19:23)]


colnames(tbl_data)


names(tbl_data)[2] = "Size"

targets = names(tbl_data)[3:12]
...
for (index in 1:length(targets)){
  ...
  tbl_data$target = log(tbl_data[[targets[index]]]+1)
  ...
  idx = which(!is.na(tbl_data$target))#(1:144)#[-122]
  ...
  plot1 = res %>% ggplot(mapping = aes(x = RapeOil, y = target, color = paste0("Size",Size), q_0.005quant = q_0.005quant, q_0.995quant = q_0.995quant)) + ggtitle(paste0("               accid ",targets[index]," model ",j," pprob ",round(max(mliks),5))) +  geom_point() +
    geom_line(aes(y = q_mode), size = 1) + expand_limits(x = 0, y = 0)+  geom_ribbon(aes(ymin = q_0.005quant, ymax = q_0.995quant, color =paste0("Size",Size)), alpha = .25) + theme(text = element_text(size = 20))  
  
  
  ggsave(plot = plot1,filename = paste0("plots_weights/weights_",targets[index],"_plot.png"))
...
ress = as.data.frame(do.call(rbind, lapply(X = 1:length(reslist),function(i){
  ...
  saveRDS(file = paste0("models_weights/",targets[i],"_weight_best.RDS"),object = (reslist[[i]]$best$summary.fixed))
...
write.csv(ress,"models_weights/final_table.csv")
