...
for (index in 1:length(targets)){
  ...
  ggsave(plot = plot1,filename = paste0("plots_plasma/plasma_",targets[index],"_plot.png"))
...
ress = as.data.frame(do.call(rbind, lapply(X = 1:5,function(i){
  probs = sort(reslist[[i]]$probs,decreasing = T)[1:2]
  bf10 = probs[1]/probs[2]
  reslist[[i]]$best$summary.fixed
  
  saveRDS(file = paste0("models_plasma/",targets[i],"_plasma_best.RDS"),object = (reslist[[i]]$best$summary.fixed))
...
write.csv(ress,"models_plasma/final_tabel.csv")
