...
targets = colnames(tbl_data)[4:19]
targets =  unlist(lapply(targets, function(tar) paste0(tar," ",unique(tbl_data$Organ))))
...
ress = as.data.frame(do.call(rbind, lapply(X = 1:length(reslist),function(i){
  ...
  saveRDS(file = paste0("models_acid/",targets[i],"_acid_best.RDS"),object = (reslist[[i]]$best$summary.fixed))
...
write.csv(ress,"models_acid/final_table.csv",row.names=T)
